...
class Camera:
    ...
    def Bright(self, Img, c, b):
        """
        """
        row, cols, channels = Img.shape
        black = np.zeros([row, cols, channels], Img.dtype)
        ImgBright = cv2.addWeighted(Img, c, black, 1-c, b)
        return ImgBright
    
    def LineTracking(self, Img, Line = 100, Dilate = 1, condition = 0, IterationErode = 10, IterationDilate = 5):
        """ 视觉寻线图像处理
        ...
        :return:
            ...
            Blur: 灰度图，方便看灰度图的结果
            WhileSum: 用于保存两个黑边中间的白色值有多少。 Key为行数，Value为对应的黑色白色像素点的值
            All: 整张图像有多少的黑色像素点的值
        """
        #下面四行是将图像变亮，c表示模糊程度（不太懂），b表示变亮的程度
#         c, b = 1.5,3
#         row, cols, channels = Img.shape
#         black = np.zeros([row, cols, channels], Img.dtype)
#         Img = cv2.addWeighted(Img, c, black, 1-c, b)
        ...
        WhileSum = {}    #用于保存两个黑边中间的白色值有多少
        All = 0   #用于记录整个图片有多少黑色像素点
        Gray = cv2.cvtColor(Img, cv2.COLOR_BGR2GRAY)    #灰度化
        Blur = cv2.GaussianBlur(Gray, (5, 5), 0)    #高斯滤波
        Retval, Dst = cv2.threshold(Blur, 40, 255, cv2.THRESH_BINARY)# | cv2.THRESH_OTSU)  #二值化
        ...
        for i in range(LineSum):    #逐行处理
            try:
                ...
                Left = where[0][0]  # 最左边符合要求的像素点
                Right = where[0][-1]  # 最右边符合要求的像素带你
                LineSum = LineSum[Left:Right]   #切割得到两边黑色的点
#                 While = np.where(Dst[i] == 255)  #白色点
#                 
#                 WhileSum[i] = len(While[0])
#                 WhileSum[i] = 0
                ...
                WhileSum[i] = 0
                pass
#             All += CentreLen[i]  #累加一下所有值
        return CentreDict, CentreLen, Dst, Blur, WhileSum, All
