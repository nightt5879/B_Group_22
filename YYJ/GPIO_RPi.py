...
#         self.P2.ChangeDutyCycle(self.DutyCycle2)
    ...
    def TurnLeft(self, DutyCycle1, DutyCycle2):
        ...
        self.DutyCycle1 = DutyCycle1
        self.DutyCycle2 = DutyCycle2
        self.P1.ChangeDutyCycle(self.DutyCycle2)
        self.P2.ChangeDutyCycle(self.DutyCycle1)
        GPIO.output(self.IN_2, GPIO.LOW)
        GPIO.output(self.IN_1, GPIO.HIGH)
        GPIO.output(self.IN_4, GPIO.HIGH)
        GPIO.output(self.IN_3, GPIO.LOW)
...
class MPU6050:
    ...
    def GetZ(self):
        GyroZ = self.ReadWordReal(0x47)/131
        if abs(GyroZ) <= 4:  #Y阈值简单滤波
            GyroZ = 0
        else:
            pass  #左边为正右边为负数
        return GyroZ
...
class PID:
    ...
    """
    def __init__(self, K, Kp, Ki, Kd, ExpVal,Kdd):
        """ 初始化PID
        输入几个关键的参数
    ...
    def TwoDin(self, NowVal, Differ):
        ...
        PID = self.K * (self.Kp * P + self.Ki * I + self.Kd * D + self.Kdd * DD)
        return PID
