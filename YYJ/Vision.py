...
class Camera:
    ...
    def LineTracking(self, Img, Line = 100,Auto = 1, Dilate = 1, condition = 0, IterationErode = 10, IterationDilate = 2):
        """ 视觉寻线图像处理
        ...
        if Auto == 1:
            Retval, Dst = cv2.threshold(Blur,45, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)  #二值化
        else:
            Retval, Dst = cv2.threshold(Blur,50, 255, cv2.THRESH_BINARY)
        if Dilate == 1:
        ...
        for i in range(LineSum):    #逐行处理
            try:
                ...
                Find = Dst[i]#[Left:Right]  #只要黑白中的数组
                Find = Find[Left:Right]
                White = np.where(Find == 255)  #黑色中间包含的白色的值下标
                WhiteSum[i] = len(White[0])   #有多少白色的值
            except:
                CentreLen[i] = 0
                CentreDict[i] = 0
                WhiteSum[i] = 0
                pass
            Sum += CentreLen[i]
        return CentreDict, CentreLen, Dst, Blur, WhiteSum, Sum
